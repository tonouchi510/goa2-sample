	...
	if vres.Pages != nil {
		...
		for i, val := range vres.Pages {
			...
			for j, val := range val.Components {
				...
				res.Pages[i].Components[j].API = unmarshalVironAPIViewToVironAPI(val.API)
		...
		for i, val := range res.Pages {
			...
			if val.Components != nil {
				...
				for j, val := range val.Components {
					...
					if val.API != nil {
						vres.Pages[i].Components[j].API = marshalVironAPIToVironAPIView(val.API)
					}
					if val.TableLabels != nil {
...
func newVironPage(vres *vironviews.VironPageView) *VironPage {
	...
	if vres.Components != nil {
		...
		for i, val := range vres.Components {
			...
			res.Components[i].API = unmarshalVironAPIViewToVironAPI(val.API)
			if val.TableLabels != nil {
...
func newVironPageView(res *VironPage) *vironviews.VironPageView {
	...
	if res.Components != nil {
		...
		for i, val := range res.Components {
			...
			if val.API != nil {
				vres.Components[i].API = marshalVironAPIToVironAPIView(val.API)
			}
			if val.TableLabels != nil {
...
// unmarshalVironAPIViewToVironAPI builds a value of type *VironAPI from a
// value of type *vironviews.VironAPIView.
func unmarshalVironAPIViewToVironAPI(v *vironviews.VironAPIView) *VironAPI {
	res := &VironAPI{
		Method: *v.Method,
		Path:   *v.Path,
	}
...
// marshalVironAPIToVironAPIView builds a value of type
// *vironviews.VironAPIView from a value of type *VironAPI.
func marshalVironAPIToVironAPIView(v *VironAPI) *vironviews.VironAPIView {
